...
from llm_council.gateway.types import GatewayRequest, GatewayResponse, CanonicalMessage, ContentBlock
...
async def test_gateway_emits_fallback_events():
    ...
    with patch("llm_council.layer_contracts.emit_layer_event") as mock_emit:
        ...
        backup = AsyncMock()
        backup.complete.return_value = GatewayResponse(content="Backup", model="model", status="ok", latency_ms=100)
...
async def test_council_emits_layer_events():
    ...
    with patch("llm_council.council.emit_layer_event") as mock_emit, \
